...
const ID_KEY_RX = /(scene(Id)?|map(Id)?|level(Id)?|area(Id)?|zone(Id)?|dungeon(Id)?|raid(Id)?|instance(Id)?|stage(Id)?|room(Id)?|copy(Id)?|chapter(Id)?)/i;
const NAME_KEY_SET = new Set(['SceneName', 'MapName', 'LevelName', 'AreaName', 'ZoneName', 'DungeonName', 'InstanceName', 'StageName', 'RoomName', 'CopyName', 'Name']);
const FLAG_KEYS = ['IsRaid', 'IsDungeon', 'IsInstance', 'InDungeon', 'InRaid', 'TeamType', 'CopyType', 'MatchType', 'Mode', 'Difficulty'];
...
export function findInterestingFields(root, { maxDepth = 6 } = {}) {
    ...
    const walk = (obj, pfx = '', depth = 0) => {
        ...
        for (const [k, v] of Object.entries(obj)) {
            ...
            if (ID_KEY_RX.test(k)) {
                ...
                if (isFiniteNum(val)) result.push({ type: 'id', key: pathKey, value: val });
            }
    ...
    walk(root);
    // tri: ids d'abord, puis flags, puis noms
    return result.sort((a, b) => {
        const prio = { id: 0, flag: 1, name: 2 };
        const ap = prio[a.type] ?? 9, bp = prio[b.type] ?? 9;
...
export function diffInteresting(prevList, currList) {
    ...
    const a = toMap(prevList), b = toMap(currList);
...
function normalizeScalar(v) {
    ...
    if (typeof v === 'object') {
        // Long from long.js
        if (v && typeof v.low === 'number' && typeof v.high === 'number' && typeof v.toString === 'function') {
            // retourner Number si sûr, sinon string
            const s = v.toString();
            const n = Number(s);
            return Number.isFinite(n) ? n : s;
        }
        return v; // laissons tel quel (string/bool/…)
    }
    return v;
}
